...
class Row extends ElementModifier {
    public params: any;
    ...
    public render() {
        let row = this.createKeyedElement({
            element: 'div', attributes: { class: 'crater-row crater-component', 'data-type': 'row' }, options: ['Edit', 'Delete', 'Clone', 'Paste'], children: [
        ...
        this.sharePoint.saveSettings(row, settings);
            ...
            element: 'div', attributes: { class: 'card sections-pane', style: { display: 'block' } }, children: [
                this.createElement({
                    element: 'div', attributes: { class: 'card-title' }, children: [
                        this.createElement({
                            element: 'h2', attributes: { class: 'title' }, text: 'Row Sections'
                        })
                    ]
                }),
            ]
    ...
    public setUpPaneContent(params) {
        ...
        // this.sharePoint.attributes.pane.content[this.key].draft.pane.content = '';
        // this.sharePoint.attributes.pane.content[this.key].content = '';
    ...
    private resetSections(params) {
        ...
        let rowSectionsContainer = this.element.find('.crater-sections-container');
        let sections = rowSectionsContainer.findAll('.crater-section');
        ...
        for (let i = 0; i < newSections.length; i++) {
            rowSectionsContainer.append(newSections[i]);
        }

        // reset count
        count = rowSectionsContainer.childNodes.length;

        rowSectionsContainer.css({ gridTemplateColumns: `repeat(${count}, 1fr` });
        let childSections = rowSectionsContainer.findAll('.crater-section');
        ...
        rowSectionsContainer.css({ gridTemplateColumns: settings.columnsSizes });
    }
    ...
    public createSections(params) {
        ...
        for (let i = 0; i < params.number; i++) {
            ...
            let element = this.createKeyedElement({
                element: 'section', attributes: { class: 'crater-component crater-container crater-section', 'data-type': 'section', style: { minHeight: params.height } }, options: ['Append', 'Edit', 'Delete', 'Clone'], type: 'crater-section', alignOptions: 'right', children: [
